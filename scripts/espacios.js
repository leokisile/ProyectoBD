// ======= CONFIG =======
const API = "http://localhost:3000/api/espacios";
...
// ======= ELEMENTOS =======
const tablaBody = document.getElementById("tblEspacios");
...
function renderTabla() {
    ...
    let filtrados = espacios.filter(e =>
        e.tipoEspacio.toLowerCase().includes(busqueda)
    );
    ...
    paginados.forEach(e => {
        tablaBody.innerHTML += `
            <tr>
                ...
                <td>${e.tipoEspacio}</td>
                <td>
                    <button class="btn-edit" onclick="editar(${e.idEspacio})">✏️</button>
                    <button class="btn-delete" onclick="eliminar(${e.idEspacio})">🗑️</button>
                </td>
            </tr>
        `;
    });
...
function editar(id) {
    const u = espacios.find(x => x.idEspacio === id);
    ...
    inputNombre.value = u.tipoEspacio;
...
function openModalNuevo() {
    ...
    if (title) title.textContent = "Nuevo Espacio";
...
async function save() {
    const data = {
        tipoEspacio: inputNombre.value.trim(),
    };

    if (!data.tipoEspacio) return toast("El tipo de espacio es obligatorio", "error");
