...
// ======= ELEMENTOS =======
const tablaBody = document.getElementById("tblCategorias");
const searchInput = document.getElementById("search");

// Campos del modal
const modalTitle = document.getElementById("modalTitle");
const inputNombre = document.getElementById("categoria");
...
function renderTabla() {
    ...
    let filtrados = categorias.filter(c =>
        c.tipoEvento.toLowerCase().includes(busqueda)
    );
    ...
    paginados.forEach(c => {
        tablaBody.innerHTML += `
            <tr>
                <td>${c.idTipoEvento}</td>
                <td>${c.tipoEvento}</td>
                <td>
                    <button class="btn-edit" onclick="editar(${c.idTipoEvento})">✏️</button>
                    <button class="btn-delete" onclick="eliminar(${c.idTipoEvento})">🗑️</button>
                </td>
            </tr>
        `;
    });
...
function openModalNuevo() {
    editandoId = null;
    modalTitle.textContent = "Nueva Categoria";
    limpiarModal();
...
function editar(id) {
    const u = categorias.find(x => x.idTipoEvento === id);
    ...
    inputNombre.value = u.tipoEvento;
...
function openModalNuevo() {
    ...
    if (title) title.textContent = "Nueva Categoria";
...
async function save() {
    const data = {
        tipoEvento: inputNombre.value.trim(),
    };

    if (!data.tipoEvento) return toast("El tipo de evento es obligatorio", "error");
